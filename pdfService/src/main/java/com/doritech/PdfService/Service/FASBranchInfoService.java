...
public class FASBranchInfoService {
    ...
    private CustomerResponse fetchCustomerDetailsByCustomerId(Integer customerId) {
        ...
        try {
            ...
            if (apiResponse.getStatusCode() == null || !apiResponse.getStatusCode().toString().equals("200")) {
                logger.warn("Customer details API returned non-200 for customerId: {}. Status: {}", customerId, apiResponse.getStatusCode());
                return null;
            }
            return new ObjectMapper().convertValue(apiResponse.getPayload(), CustomerResponse.class);
        } catch (feign.FeignException fe) {
            logger.error("Feign error fetching customer details for customerId: {}. Error: {}", customerId, fe.getMessage());
            return null;
        } catch (Exception e) {
    ...
    private CustomerResponse fetchCustomerPayload(Integer scheduleVisitId) {
        ...
        try {
            ...
            if (res.getStatusCode() == null || !res.getStatusCode().toString().equals("200")) {
                logger.warn("Customer API returned non-200 for scheduleVisitId: {}. Status: {}", scheduleVisitId, res.getStatusCode());
                return null;
            }
            return new ObjectMapper().convertValue(res.getPayload(), CustomerResponse.class);
        } catch (feign.FeignException fe) {
            logger.error("Feign error fetching customer for scheduleVisitId: {}. Error: {}", scheduleVisitId, fe.getMessage());
            return null;
        } catch (Exception e) {
    ...
    private void updateAssignmentStatusToCompleted(Integer scheduleVisitId) {
        if (scheduleVisitId == null) {
            logger.warn("updateAssignmentStatusToCompleted called with null scheduleVisitId, skipping.");
            return;
        }
        try {
            int rows = jdbcTemplate.update(
                "UPDATE employee_assignment SET status = 'Completed', modified_on = NOW() WHERE assignment_id = ?",
                scheduleVisitId
            );
            if (rows > 0) {
                logger.info("Assignment status updated to COMPLETED for assignment_id: {}", scheduleVisitId);
            } else {
                logger.warn("No rows updated for assignment_id: {}. Record may not exist.", scheduleVisitId);
            }
        } catch (Exception e) {
    ...
    public FASBranchInfoResponseDTO createFASInfoAndGeneratePdf(FASBranchInfoRequestDTO requestDTO,
        ...
        if (optional.isPresent()) {
            ...
            FASBranchInfoResponseDTO responseDTO = mapEntityToResponseDto(existingEntity, customerDetailsPayload, customerPayload);
    ...
    public FASBranchInfoResponseDTO getFASInfoById(Long id) {
        ...
        if (customerPayload == null)
            logger.warn("Could not fetch customer payload for scheduleVisitId: {} (ID: {}). Proceeding with stored data.", entity.getScheduleVisitId(), id);
        return mapEntityToResponseDto(entity, customerDetailsPayload, customerPayload);
    }

    public FASBranchInfoResponseDTO getFASInfoByScheduleVisitAndProductType(Integer scheduleVisitId, String productType) {
        ...
        if (customerPayload == null)
            logger.warn("Could not fetch customer payload for scheduleVisitId: {} during getByScheduleVisit.", scheduleVisitId);
        return mapEntityToResponseDto(entity, customerDetailsPayload, customerPayload);
    }

    public Page<FASBranchInfoResponseDTO> getAllFASInfo(int page, int size, String sortBy, String sortDir) {
        logger.info("Fetching all FAS branch info - page: {}, size: {}, sortBy: {}, sortDir: {}", page, size, sortBy, sortDir);
        ...
        return fasBranchInfoRepository.findAll(pageable).map(entity -> {
            try {
                ...
                if (customerPayload == null)
                    logger.warn("Could not fetch customer payload for scheduleVisitId: {} (ID: {}) during getAll.", entity.getScheduleVisitId(), entity.getId());
    ...
    private byte[] regenerateAndSavePdf(FASBranchInfo entity) throws Exception {
        ...
        if (customerPayload == null)
            logger.warn("Could not fetch customer payload for PDF regeneration. ID: {}, scheduleVisitId: {}.", entity.getId(), entity.getScheduleVisitId());
    ...
    private FASBranchInfo generateAndSavePdfFile(FASBranchInfo saved, FASBranchInfoRequestDTO enriched) {
        try {
            ...
            updateAssignmentStatusToCompleted(saved.getScheduleVisitId());
        } catch (Exception e) {
    ...
    public ByteArrayOutputStream generateFASServiceReport(FASBranchInfoRequestDTO dto) throws Exception {
        ...
        cp.add(new Chunk("SCO 6-7, Saundh Complex, Near Atwal House, Cantt Road, Jalandhar-144005 (Punjab), Ph: 0181-4634072\n", smallBoldFont));
        ...
        addFormRow(leftTable, "Name of Field Associate:", safeStr(dto, FASBranchInfoRequestDTO::getFieldAssociateName), normalFont, valueBlue);
        addFormRow(leftTable, "Branch/Site Name:", safeStr(dto, FASBranchInfoRequestDTO::getBranchName), normalFont, valueBlue);
        addFormRow(leftTable, "Under Regional Office/Zonal Office:", safeStr(dto, FASBranchInfoRequestDTO::getRegionalOffice), normalFont, valueBlue);
        addFormRow(leftTable, "Address:", safeStr(dto, FASBranchInfoRequestDTO::getBranchAddress), normalFont, valueBlue);
        ...
        addFormRow(leftTable, "Phone / Mobile No:-", safeStr(dto, FASBranchInfoRequestDTO::getConcernedPersonPhone), normalFont, valueBlue);
        ...
        addRightRow(rightTable, "NO. OF ZONES", safeStr(dto, FASBranchInfoRequestDTO::getNoOfZones), normalFont, valueBlue);

        String instDateStr = dto != null && dto.getDateOfInstallation() != null ? DATE_FMT.format(dto.getDateOfInstallation()) : "";
        addRightRow(rightTable, "DATE OF INSTALLATION", instDateStr, normalFont, valueBlue);
        addRightRow(rightTable, "BATTERY VOLTAGE", safeStr(dto, FASBranchInfoRequestDTO::getBatteryVoltage) + " VOLTS", normalFont, valueBlue);
        ...
        addStatusRow(statusTable, "System Connected Through", smallBoldFont,
                dto != null && dto.getSystemConnectedThrough() != null ? dto.getSystemConnectedThrough().trim().toLowerCase() : "",
        ...
        addStatusRow(statusTable, "Auto-dialer [PSTN/GSM]", smallBoldFont,
                dto != null && dto.getAutoDialerTypePstnGsm() != null ? dto.getAutoDialerTypePstnGsm().trim().toLowerCase() : "",
        ...
        addStatusRow(statusTable, "Auto-dialer Type", smallBoldFont,
                dto != null && dto.getAutoDialerInbuiltExternal() != null ? dto.getAutoDialerInbuiltExternal().trim().toLowerCase() : "",
        ...
        String noVal = dto != null && dto.getNoUpdatedInAutoDialer() != null ? String.valueOf(dto.getNoUpdatedInAutoDialer()) : "";
        ...
        List<FASHardwareItemDTO> hwItems = dto != null && dto.getHardwareItems() != null ? dto.getHardwareItems() : Collections.emptyList();
        for (FASHardwareItemDTO hw : hwItems) {
            try {
                PdfPCell snCell = new PdfPCell(new Phrase(hw.getSerialNumber() != null ? String.valueOf(hw.getSerialNumber()) : "", tiny));
                snCell.setBorder(Rectangle.BOX);
                snCell.setPadding(3);
                snCell.setHorizontalAlignment(Element.ALIGN_CENTER);
                hwTable.addCell(snCell);

                String itemLabel = hw.getZoneLabel() != null && !hw.getZoneLabel().isBlank() ? hw.getZoneLabel() : (hw.getItemName() != null ? hw.getItemName() : "");
                PdfPCell nameCell = new PdfPCell(new Phrase(itemLabel, hw.getZoneLabel() != null ? valueBlueTiny : tiny));
                ...
                PdfPCell qtyCell = new PdfPCell(new Phrase(hw.getQuantity() != null ? String.valueOf(hw.getQuantity()) : "", valueBlueTiny));
                ...
                PdfPCell locCell = new PdfPCell(new Phrase(hw.getLocation() != null ? hw.getLocation() : "", valueBlueTiny));
        ...
        for (String dh : new String[]{"Name of Staff / Authority", "Mobile/Tel. Number", "Name of Staff / Authority", "Mobile/Tel. Number"}) {
        ...
        List<FASDialerContactDTO> contacts = dto != null && dto.getDialerContacts() != null ? dto.getDialerContacts() : Collections.emptyList();
        for (int i = 0; i < Math.max(contacts.size(), 2); i += 2) {
            try {
                ...
                PdfPCell ln = new PdfPCell(new Phrase(left != null && left.getStaffName() != null ? left.getStaffName() : "", valueBlueTiny));
                ln.setBorder(Rectangle.BOX);
                ln.setPadding(3);
                ln.setHorizontalAlignment(Element.ALIGN_CENTER);
                dialerTable.addCell(ln);

                PdfPCell lm = new PdfPCell(new Phrase(left != null && left.getMobileNumber() != null ? left.getMobileNumber() : "", valueBlueTiny));
                lm.setBorder(Rectangle.BOX);
                lm.setPadding(3);
                lm.setHorizontalAlignment(Element.ALIGN_CENTER);
                dialerTable.addCell(lm);

                PdfPCell rn = new PdfPCell(new Phrase(right != null && right.getStaffName() != null ? right.getStaffName() : "", valueBlueTiny));
                rn.setBorder(Rectangle.BOX);
                rn.setPadding(3);
                rn.setHorizontalAlignment(Element.ALIGN_CENTER);
                dialerTable.addCell(rn);

                PdfPCell rm = new PdfPCell(new Phrase(right != null && right.getMobileNumber() != null ? right.getMobileNumber() : "", valueBlueTiny));
        ...
        Boolean[] gcpStatuses = {
            dto != null ? dto.getIsPanelCleaned() : null,
            dto != null ? dto.getIsOperationExplained() : null,
            dto != null ? dto.getAreHootersDifficultToAccess() : null,
            dto != null ? dto.getIsSettingDoneAsPerBank() : null,
            dto != null ? dto.getAreSensorsCleaned() : null,
            dto != null ? dto.getIsPanelAndSensorsCleaned() : null,
            dto != null ? dto.getIsBatteryWorking() : null,
            dto != null ? dto.getIsPasswordChanged() : null
        };
        String[] gcpLabels = {
            "1. Fire Alarm Panel cleaned from inside & outside",
            ...
            "4. BATTERY CHECKED & FOUND TO BE WORKING PROPERLY",
        ...
        List<FASMaterialDTO> allMaterials = dto != null && dto.getMaterials() != null ? dto.getMaterials() : Collections.emptyList();
        ...
        matSection.addCell(new PdfPCell(buildMaterialTable("MATERIAL REPLACED", replaced, tinyBold, valueBlueTiny, checkIcon, crossIcon)));
        matSection.addCell(new PdfPCell(buildMaterialTable("MATERIAL REQUIRED", required, tinyBold, valueBlueTiny, checkIcon, crossIcon)));
        ...
        PdfPCell defHeader = new PdfPCell(new Phrase("Defect found on inspection / Remarks / Recommendation by Field Associate", tinyBold));
        ...
        PdfPCell remarksCell = new PdfPCell(new Phrase(dto != null && dto.getFaRemarks() != null ? dto.getFaRemarks() : "", valueBlueTiny));
        remarksCell.setFixedHeight(50f);
        remarksCell.setPadding(4);
        remarksCell.setBorder(Rectangle.BOX);
        leftBottom.addCell(remarksCell);

        PdfPCell syHeader = new PdfPCell(new Phrase("System is working properly & is in ON condition. It has been explained to:", tinyBold));
        ...
        List<FASStaffMemberDTO> staffList = dto != null && dto.getStaffMembers() != null ? dto.getStaffMembers() : Collections.emptyList();
        for (int i = 0; i < staffList.size(); i++) {
            try {
                ...
                PdfPCell s3 = new PdfPCell(new Phrase(st.getDesignation() != null ? st.getDesignation() : "", valueBlueTiny));
        ...
        PdfPCell crVal = new PdfPCell(new Phrase(dto != null && dto.getCustomerRemarks() != null ? dto.getCustomerRemarks() : "", valueBlueTiny));
        ...
        billTable.addCell(createStatusCell(valueBlueTiny, dto != null ? dto.getBillToBeRaised() : null, checkIcon, crossIcon));
        ...
        billTable.addCell(createStatusCell(valueBlueTiny, dto != null ? dto.getIsInvoicePaymentFollowed() : null, checkIcon, crossIcon));
        ...
        prPh.add(new Chunk(dto != null && dto.getPaymentRemarks() != null ? dto.getPaymentRemarks() : "", valueBlueTiny));
        ...
        PdfPCell bankCell = new PdfPCell(new Phrase(
    ...
    private void addStatusRow(PdfPTable table, String labelText, Font labelFont, String value,
            String[] options, Font valueFont, Image checkIcon, Image crossIcon) throws Exception {
    ...
    private PdfPCell createMultiOptionStatusCell(Font font, String value, String[] options, Image checkIcon, Image crossIcon) throws Exception {
            ...
            cell.setCellEvent((c, pos, canvases) -> {
                try {
                    ...
                    if (fi != null) {
                        ...
                        fi.setAbsolutePosition((pos.getLeft() + pos.getRight()) / 2 - fi.getScaledWidth() / 2, pos.getBottom() + 2);
    ...
    private PdfPTable buildMaterialTable(String headerText, List<FASMaterialDTO> items, Font hFont, Font vFont, Image checkIcon, Image crossIcon) throws Exception {
        ...
        for (int i = 0; i < items.size(); i++) {
            try {
                ...
                PdfPCell desc = new PdfPCell(new Phrase(m.getItemDescription() != null ? m.getItemDescription() : "", vFont));
                desc.setBorder(Rectangle.BOX);
                desc.setPadding(3);
                t.addCell(desc);
                ...
                PdfPCell qty = new PdfPCell(new Phrase(m.getQuantity() != null ? String.valueOf(m.getQuantity()) : "", vFont));
    ...
    private FASBranchInfoRequestDTO enrichDto(FASBranchInfoRequestDTO dto, CustomerResponse customerDetailsPayload, CustomerResponse customerPayload) {
        try {
            if (customerDetailsPayload != null) {
            ...
            logger.warn("Error enriching FAS request DTO with Feign data. Continuing with available data. Error: {}", e.getMessage());
        }
        return dto;
    }
    ...
    private FASBranchInfoResponseDTO mapEntityToResponseDto(FASBranchInfo e, CustomerResponse customerDetailsPayload, CustomerResponse customerPayload) {
        ...
        if (customerDetailsPayload != null) {
            ...
            logger.warn("customerDetailsPayload is null for FAS entity ID: {}. Branch fields will be empty.", e.getId());
        }
    ...
    private FASBranchInfoResponseDTO mapEntityToResponseDtoList(FASBranchInfo e, CustomerResponse customerDetailsPayload, CustomerResponse customerPayload) {
        FASBranchInfoResponseDTO dto = new FASBranchInfoResponseDTO();
        dto.setId(e.getId());
        dto.setCustomerId(e.getCustomerId());
        dto.setScheduleVisitId(e.getScheduleVisitId());
