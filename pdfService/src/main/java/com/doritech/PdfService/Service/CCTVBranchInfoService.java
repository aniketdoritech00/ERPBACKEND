...
public class CCTVBranchInfoService {
	...
	private CustomerResponse fetchCustomerDetailsByCustomerId(Integer customerId) {
		...
		try {
			...
			if (apiResponse.getStatusCode() == null || !apiResponse.getStatusCode().toString().equals("200")) {
				logger.warn("Customer details API returned non-200 status for customerId: {}. Status: {}", customerId,
	...
	private CustomerResponse fetchCustomerPayload(Integer scheduleVisitId) {
		...
		try {
			...
			if (customerResponse.getStatusCode() == null
					|| !customerResponse.getStatusCode().toString().equals("200")) {
				logger.warn("Customer API returned non-200 status for scheduleVisitId: {}. Status: {}", scheduleVisitId,
	...
	public CCTVBranchInfoResponseDTO createCCTVInfoAndGeneratePdf(CCTVBranchInfoRequestDTO requestDTO,
		...
		logger.info("Starting CCTV branch info creation for branch: {}",
				requestDTO != null ? requestDTO.getBranchName() : "null");
		...
		if (optional.isPresent()) {
			logger.warn("CCTV data already exists for scheduleVisitId: {} and productType: {}",
		...
		CCTVBranchInfoRequestDTO enrichedDto = enrichRequestDtoWithFeignData(requestDTO, customerDetailsPayload,
				customerPayload);
		...
		try {
			...
			if (baos != null) {
				...
				updateAssignmentStatusToCompletedFromRepo(savedCCTVInfo.getScheduleVisitId());
			}
		} catch (Exception e) {
			logger.error("PDF generation failed for ID: {}. Error: {}", savedCCTVInfo.getId(), e.getMessage(), e);
		}

		CCTVBranchInfoResponseDTO responseDTO = mapEntityToResponseDto(savedCCTVInfo, customerDetailsPayload,
				customerPayload);
	...
	private CCTVBranchInfoRequestDTO enrichRequestDtoWithFeignData(CCTVBranchInfoRequestDTO requestDTO,
		...
		try {
			...
			logger.warn("Error enriching request DTO with Feign data. Continuing with available data. Error: {}",
					e.getMessage());
		}
		return requestDTO;
	}
	...
	private void saveDocuments(CCTVBranchInfo savedCCTVInfo, Map<String, MultipartFile> allFiles) {
		...
		allFiles.entrySet().stream().filter(entry -> !entry.getKey().equals("data")).forEach(entry -> {
	...
	public CCTVBranchInfoResponseDTO getCCTVInfoById(Long id) {
		...
		try {
			...
			if (customerDetailsPayload == null) {
				logger.warn(
						"Could not fetch customer details for customerId: {} (ID: {}). Proceeding with stored data.",
						entity.getCustomerId(), id);
			}
			if (customerPayload == null) {
				logger.warn(
						"Could not fetch customer details for scheduleVisitId: {} (ID: {}). Proceeding with stored data.",
	...
	public CCTVBranchInfoResponseDTO getCCTVInfoByScheduleVisitAndProductType(Integer scheduleVisitId,
			String productType) {
		...
		try {
			...
			if (customerPayload == null) {
				logger.warn("Could not fetch customer details for scheduleVisitId: {} during getByScheduleVisit.",
						scheduleVisitId);
			}

			logger.info("Successfully fetched CCTV info for scheduleVisitId: {} and productType: {}", scheduleVisitId,
					productType);
			return mapEntityToResponseDto(entity, customerDetailsPayload, customerPayload);

		} catch (RuntimeException e) {
			logger.error("RuntimeException fetching CCTV info. scheduleVisitId: {}, productType: {}, error: {}",
					scheduleVisitId, productType, e.getMessage(), e);
			throw e;
		} catch (Exception e) {
			logger.error("Unexpected error fetching CCTV info. scheduleVisitId: {}, productType: {}, error: {}",
					scheduleVisitId, productType, e.getMessage(), e);
	...
	public Page<CCTVBranchInfoResponseDTO> getAllCCTVInfo(int page, int size, String sortBy, String sortDir) {
		logger.info("Fetching all CCTV branch info - page: {}, size: {}, sortBy: {}, sortDir: {}", page, size, sortBy,
				sortDir);
		try {
			...
			Page<CCTVBranchInfoResponseDTO> responsePage = cctvPage.map(entity -> {
				try {
					...
					if (customerPayload == null) {
						logger.warn("Could not fetch customer details for scheduleVisitId: {} (ID: {}) during getAll.",
								entity.getScheduleVisitId(), entity.getId());
					}
					return mapEntityToListResponseDto(entity, customerDetailsPayload, customerPayload);
				} catch (Exception e) {
					logger.error("Error mapping entity ID: {} during getAll. Error: {}", entity.getId(), e.getMessage(),
							e);
					return mapEntityToListResponseDto(entity, null, null);
				}
			});

			logger.info("Successfully fetched {} CCTV records (page {}/{})", responsePage.getNumberOfElements(),
					page + 1, cctvPage.getTotalPages());
			return responsePage;

		} catch (Exception e) {
			logger.error("Unexpected error in getAllCCTVInfo - page: {}, size: {}. Error: {}", page, size,
					e.getMessage(), e);
	...
	public ByteArrayOutputStream generateCCTVServiceReport(CCTVBranchInfoRequestDTO dto) throws Exception {
		...
		try {
			...
			addFormRow(leftTable, "Name of Field Associate :",
					dto != null && dto.getFieldAssociateName() != null ? dto.getFieldAssociateName() : "", normalFont,
					valueBlueFont);
			addFormRow(leftTable, "Branch/Site Name :",
					dto != null && dto.getBranchName() != null ? dto.getBranchName() : "", normalFont, valueBlueFont);
			addFormRow(leftTable, "Under Regional Office/Zonal Office :",
					dto != null && dto.getRegionalOffice() != null ? dto.getRegionalOffice() : "", normalFont,
					valueBlueFont);
			addFormRow(leftTable, "Address:-",
					dto != null && dto.getBranchAddress() != null ? dto.getBranchAddress() : "", normalFont,
					valueBlueFont);
			...
			String concernedDesignation = dto != null && dto.getConcernedPersonDesignation() != null
					? dto.getConcernedPersonDesignation()
					: "";
			...
			addFormRow(leftTable, "Phone / Mobile No:-",
					dto != null && dto.getConcernedPersonPhone() != null ? dto.getConcernedPersonPhone() : "",
					normalFont, valueBlueFont);
			addFormRow(leftTable, "Customer Name:-",
					dto != null && dto.getCustomerName() != null ? dto.getCustomerName() : "", normalFont,
					valueBlueFont);
			addFormRow(leftTable, "District:-", dto != null && dto.getDistrict() != null ? dto.getDistrict() : "",
					normalFont, valueBlueFont);
			...
			if (dto != null && dto.getInspectionDate() != null) {
				try {
					...
					logger.warn("Failed to format inspection date: {}. Error: {}", dto.getInspectionDate(),
							ex.getMessage());
			...
			if (dto != null && dto.getCctvProductDetail() != null) {
				addRightFormRow(rightTable, "DVR/NVR Make", dto.getCctvProductDetail().getDvrNvrMake(), normalFont,
						valueBlueFont);
				addRightFormRow(rightTable, "DVR/NVR Model", dto.getCctvProductDetail().getDvrNvrModel(), normalFont,
						valueBlueFont);
				...
				try {
					...
					builtDateStr = dto.getCctvProductDetail().getBuiltDate() != null
							? dto.getCctvProductDetail().getBuiltDate().toString()
							: "";
				}
				...
				addRightFormRow(rightTable, "Total Camera Port",
						dto.getCctvProductDetail().getTotalCameraPort() != null
								? String.valueOf(dto.getCctvProductDetail().getTotalCameraPort())
								: "",
						normalFont, valueBlueFont);
				...
				try {
					...
					installationDateStr = dto.getCctvProductDetail().getDateOfInstallation() != null
							? dto.getCctvProductDetail().getDateOfInstallation().toString()
							: "";
				}
			...
			boolean isWorking = dto != null && dto.getCctvWorkingStatus() != null
					&& dto.getCctvWorkingStatus().equalsIgnoreCase("true");
			...
			yBox.setCellEvent(new PdfPCellEvent() {
				...
				public void cellLayout(PdfPCell cell, Rectangle position, PdfContentByte[] canvases) {
					...
					float offset = 2f;
					cb.rectangle(position.getLeft() + offset, position.getBottom() + offset,
							position.getWidth() - 2 * offset, position.getHeight() - 2 * offset);
					cb.stroke();
			...
			List<CCTVProductInstalledStatusDTO> cctvProductInstalledStatuses = dto != null
					? dto.getCctvProductInstalledStatus()
					: null;
			int serial = 1;
				...
				for (CCTVProductInstalledStatusDTO item : cctvProductInstalledStatuses) {
					try {
						...
						PdfPCell cell4 = new PdfPCell(
								new Phrase(item.getDetails() != null ? item.getDetails() : "", valueBlueFont));
						...
						if (item.getSubItems() != null && !item.getSubItems().isEmpty()) {
							for (CCTVProductInstalledSubStatusDTO sub : item.getSubItems()) {
								try {
									...
									if (sub.getItemName() != null && (sub.getItemName().toLowerCase().contains("dvr")
											|| sub.getItemName().toLowerCase().contains("nvr"))) {
										...
										if (safetyObj instanceof Boolean) {
											...
											safetyStatus = s.equals("true") || s.equals("yes") ? "Yes"
													: s.equals("false") || s.equals("no") ? "No" : "N.A";
										} else {
											safetyStatus = "N.A";
										}
									...
									logger.error("Error rendering sub-item row for item: {}. Error: {}",
											sub.getItemName(), e.getMessage(), e);
								}
							}
						}
					} catch (Exception e) {
						logger.error("Error rendering product installed status row for category: {}. Error: {}",
								item.getItemCategory(), e.getMessage(), e);
					}
				}
			} else {
				logger.warn("cctvProductInstalledStatuses is null for branch: {}",
			...
			String[] camHeaders = { "S.No", "Camera Location", "Type", "Working Status", "S.No", "Camera Location",
					"Type", "Working Status" };
			...
			List<CCTVCameraStatusDTO> camList = dto != null && dto.getCctvCameraStatus() != null
					? dto.getCctvCameraStatus()
					: Collections.emptyList();

			Integer totalCameraPort = dto != null && dto.getCctvProductDetail() != null
					&& dto.getCctvProductDetail().getTotalCameraPort() != null
							? dto.getCctvProductDetail().getTotalCameraPort()
							: camList.size();
			...
			for (int i = 0; i < totalCameraPort; i += 2) {
				try {
					...
					PdfPCell camLocA = new PdfPCell(
							new Phrase(camA != null && camA.getCameraLocation() != null ? camA.getCameraLocation()
									: "Camera " + (i + 1), valueBlueFontTiny));
					...
					PdfPCell typeA = new PdfPCell(
							new Phrase(camA != null && camA.getCameraType() != null ? camA.getCameraType() : "",
									valueBlueFontTiny));
					...
					if (i + 1 < totalCameraPort) {
						...
						PdfPCell camLocB = new PdfPCell(
								new Phrase(camB != null && camB.getCameraLocation() != null ? camB.getCameraLocation()
										: "Camera " + (i + 2), valueBlueFontTiny));
						...
						PdfPCell typeB = new PdfPCell(
								new Phrase(camB != null && camB.getCameraType() != null ? camB.getCameraType() : "",
										valueBlueFontTiny));
						...
						PdfPCell statusB = createSmallStatusCell(tiny,
								camB != null ? camB.getCameraWorkingStatus() : null);
			...
			String totalHddSlotsStr = dto != null && dto.getTotalHddSlots() != null
					? String.valueOf(dto.getTotalHddSlots())
					: "N/A";
			String hddInstalledQtyStr = dto != null && dto.getHddInstalledQty() != null
					? String.valueOf(dto.getHddInstalledQty())
					: "N/A";
			String totalCapacityStr = dto != null && dto.getTotalCapacityTb() != null
					? String.valueOf(dto.getTotalCapacityTb())
					: "N/A";
			String freeSpaceStr = dto != null && dto.getTotalFreeSpaceTb() != null
					? String.valueOf(dto.getTotalFreeSpaceTb())
					: "N/A";
			...
			List<CCTVHddStatusDTO> hddList = dto != null && dto.getCctvHddStatus() != null ? dto.getCctvHddStatus()
					: Collections.emptyList();
			...
			for (int i = 0; i < totalHddSlots; i++) {
				try {
					...
					PdfPCell capacity = new PdfPCell(new Phrase(
							hdd != null && hdd.getCapacity() != null ? hdd.getCapacity() : "", valueBlueFontTiny));
			...
			String[][] rows = { { "1. AC supply is coming uninterrupted." }, { "2. System is connected to UPS." },
			...
			Boolean[] statuses = new Boolean[] { dto != null ? dto.getIsAcSupplyUninterrupted() : null,
					dto != null ? dto.getIsSystemOnUps() : null, dto != null ? dto.getAreAllCamerasFunctional() : null,
					dto != null ? dto.getIsRecordingAvailable() : null,
					dto != null ? dto.getIsOperationExplainedAndUnderstood() : null,
					dto != null ? dto.getIsMotionDetectionSet() : null, dto != null ? dto.getIsSystemCleaned() : null,
			...
			for (int i = 0; i < rows.length; i++) {
				try {
					...
					logger.error("Error rendering general checkpoint row at index: {}. Error: {}", i, e.getMessage(),
							e);
				}
			}
			...
			List<CCTVMaterialDTO> allMaterials = dto != null && dto.getMaterials() != null ? dto.getMaterials()
					: Collections.emptyList();
			...
			for (int i = 0; i < replacedMaterials.size(); i++) {
				try {
					...
					PdfPCell mat = new PdfPCell(new Phrase(
							matDto.getItemDescription() != null ? matDto.getItemDescription() : "", valueBlueFontTiny));
					mat.setPadding(3);
					...
					PdfPCell qtyCell = new PdfPCell(
							new Phrase(matDto.getQuantity() != null ? String.valueOf(matDto.getQuantity()) : "",
									valueBlueFontTiny));
					qtyCell.setHorizontalAlignment(Element.ALIGN_CENTER);
			...
			PdfPCell defectFound = new PdfPCell(
			...
			rightHalfPhrase.add(new Chunk(
					dto != null && dto.getEstimateProductDetails() != null ? dto.getEstimateProductDetails() : "",
					valueBlueFontTiny));
			...
			String estimateAmountValue = dto != null && dto.getEstimateAmount() != null
					? String.valueOf(dto.getEstimateAmount())
					: "";
			...
			PdfPCell billYNWrapperInner = createStatusCell(valueBlueFontTiny,
					dto != null ? dto.getBillToBeRaised() : null);
			...
			PdfPCell systemWorking = new PdfPCell(new Phrase(
					"System is working properly & is in OIN condition. It has been explained to:", tinyBold));
			...
			List<CCTVStaffMemberDTO> staffList = dto != null && dto.getStaffMembers() != null ? dto.getStaffMembers()
					: Collections.emptyList();
			...
			for (int i = 0; i < staffList.size(); i++) {
				try {
					...
					PdfPCell nameCell2 = new PdfPCell(
							new Phrase(staff.getName() != null ? staff.getName() : "", valueBlueFontTiny));
					nameCell2.setPadding(3);
					explainedToTable.addCell(nameCell2);

					PdfPCell desigCell = new PdfPCell(new Phrase(
							staff.getDesignation() != null ? staff.getDesignation() : "", valueBlueFontTiny));
			...
			PdfPCell pendingInvoiceWrapperInner = createStatusCell(valueBlueFontTiny,
					dto != null ? dto.getIsInvoicePaymentFollowed() : null);
			...
			PdfPCell companyNameContent = new PdfPCell(
					new Phrase("Digitals India Security Products Pvt Ltd", tinyBold));
			...
			PdfPCell bankDetails = new PdfPCell(
					new Phrase("Do not make cash payment, Please transfer payment in the following company account\n"
			...
			PdfPCell disclaimer = new PdfPCell(
					new Phrase("THIS IS NOT A VALID RECEIPT FOR REMOVING MATERIAL FROM YOUR BRANCH. "
			...
			logger.info("PDF generation completed successfully for branch: {}",
					dto != null ? dto.getBranchName() : "null");

		} catch (Exception e) {
			logger.error("Critical error during PDF generation for branch: {}. Error: {}",
					dto != null ? dto.getBranchName() : "null", e.getMessage(), e);
	...
	private PdfPCell createSmallStatusCell(Font font, Object statusObj) {
		try {
			if (checkIcon == null)
				checkIcon = loadIconFromIconsDir("check");
			if (crossIcon == null)
				crossIcon = loadIconFromIconsDir("cross");
			if (naIcon == null)
				naIcon = loadIconFromIconsDir("check");
				...
				cell.setCellEvent(new PdfPCellEvent() {
					...
					public void cellLayout(PdfPCell cell, Rectangle position, PdfContentByte[] canvases) {
						try {
							...
							logger.error("Error in createSmallStatusCell cellLayout for text '{}': {}", finalText,
									e.getMessage(), e);
						}
					}
				});
				...
				cell.setCellEvent(new PdfPCellEvent() {
					...
					public void cellLayout(PdfPCell cell, Rectangle position, PdfContentByte[] canvases) {
						try {
							...
							logger.error("Error in createStatusCell cellLayout for text '{}': {}", finalText,
									e.getMessage(), e);
						}
					}
				});
		...
		try {
			...
			if (dto.getCctvProductDetail() != null) {
				try {
					...
					logger.error("Error mapping CCTV product detail for branch '{}': {}", dto.getBranchName(),
							e.getMessage(), e);
				}
			}
			...
			if (dto.getCctvCameraStatus() != null) {
				try {
					...
					logger.error("Error mapping CCTV camera status list for branch '{}': {}", dto.getBranchName(),
							e.getMessage(), e);
				}
			}
			...
			if (dto.getCctvHddStatus() != null) {
				try {
					...
					logger.error("Error mapping CCTV HDD status list for branch '{}': {}", dto.getBranchName(),
							e.getMessage(), e);
				}
			}
					...
					List<CCTVMaterial> materialsList = dto.getMaterials().stream().map(materialDto -> {
						...
						if (materialDto.getMaterialType() != null) {
							material.setMaterialType(
									CCTVMaterial.MaterialType.valueOf(materialDto.getMaterialType().name()));
						}
						return material;
					}).collect(Collectors.toList());
					entity.setCctvMaterials(materialsList);
				} catch (Exception e) {
					logger.error("Error mapping CCTV materials list for branch '{}': {}", dto.getBranchName(),
							e.getMessage(), e);
				}
			}
			...
			if (dto.getStaffMembers() != null) {
				try {
					...
					logger.error("Error mapping staff members list for branch '{}': {}", dto.getBranchName(),
							e.getMessage(), e);
				}
			}
			...
			if (dto.getCctvProductInstalledStatus() != null) {
				try {
					List<CCTVProductInstalledStatus> statusList = dto.getCctvProductInstalledStatus().stream()
							.map(statusDto -> {
								...
								if (statusDto.getSubItems() != null) {
									try {
										List<CCTVProductInstalledSubStatus> subList = statusDto.getSubItems().stream()
												.map(subDto -> {
										...
										logger.error("Error mapping sub-items for category '{}': {}",
												statusDto.getItemCategory(), e.getMessage(), e);
									}
								}
								return status;
							}).collect(Collectors.toList());
					...
					logger.error("Error mapping CCTV installed product statuses for branch '{}': {}",
							dto.getBranchName(), e.getMessage(), e);
				}
			}
			...
			logger.error("Critical error in mapDtoToEntity for branch '{}': {}",
					dto != null ? dto.getBranchName() : "null", e.getMessage(), e);
		...
		try {
			...
			if (entity.getCctvProductDetail() != null) {
				try {
					...
					logger.error("Error mapping product detail to DTO for ID '{}': {}", entity.getId(), e.getMessage(),
							e);
				}
			}
			...
			if (entity.getCctvCameraStatus() != null) {
				try {
					...
					logger.error("Error mapping camera status to DTO for ID '{}': {}", entity.getId(), e.getMessage(),
							e);
				}
			}
				...
				try {
					dto.setMaterials(entity.getCctvMaterials().stream().map(material -> {
						...
						if (material.getMaterialType() != null) {
							materialDto.setMaterialType(
									CCTVMaterialDTO.MaterialType.valueOf(material.getMaterialType().name()));
						}
						return materialDto;
					}).collect(Collectors.toList()));
				} catch (Exception e) {
			...
			if (entity.getCctvStaffMembers() != null) {
				try {
					...
					logger.error("Error mapping staff members to DTO for ID '{}': {}", entity.getId(), e.getMessage(),
							e);
				}
			}
					...
					dto.setCctvProductInstalledStatus(entity.getCctvProductInstalledStatuses().stream().map(status -> {
						...
						if (status.getSubItems() != null) {
							try {
								...
								logger.error("Error mapping sub-items to DTO for status category '{}': {}",
										status.getItemCategory(), e.getMessage(), e);
							}
						}
						return statusDto;
					}).collect(Collectors.toList()));
				} catch (Exception e) {
					logger.error("Error mapping installed product statuses to DTO for ID '{}': {}", entity.getId(),
							e.getMessage(), e);
				}
			}
			...
			if (entity.getCctvBranchDocuments() != null && !entity.getCctvBranchDocuments().isEmpty()) {
				try {
					List<CCTVBranchDocument> docs = cctvBranchDocumentRepository
							.findDocumentsByBranchInfoId(entity.getId());
					if (docs != null && !docs.isEmpty()) {
						...
						List<CCTVBranchDocumentDTO> documentDTOs = docs.stream().filter(Objects::nonNull).map(doc -> {
							try {
								...
								logger.error("Error mapping document ID: {}, Error: {}", doc.getDocumentId(),
										e.getMessage());
								return null;
							}
			...
			logger.error("Critical error in mapEntityToResponseDto for ID '{}': {}",
	...
	private CCTVBranchInfoResponseDTO mapEntityToListResponseDto(CCTVBranchInfo entity,
		...
		try {
			...
			logger.error("Error mapping CCTVBranchInfo to list DTO for ID: {}. Error: {}", entity.getId(),
					e.getMessage(), e);
			throw e;
		}
	}
	...
	private byte[] regenerateAndSavePdf(CCTVBranchInfo entity) {
		...
		try {
			...
			if (customerPayload == null) {
				logger.warn("Could not fetch customer details for PDF regeneration. ID: {}, scheduleVisitId: {}.",
			...
			try {
				...
				logger.warn("Could not save regenerated PDF to disk for ID: {}. Error: {}", entity.getId(),
						e.getMessage());
			}
